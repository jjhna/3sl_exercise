...
public class King : Imovement
{
    ...
    */
    ...
    /*
        ...
        position, so for the King, we need to see if the new target position is within one tile , if so then
    ...
    */
    private bool positionCheck(Vector2Int pos, Vector2Int newPos) {
        //The king can only move in 8 directions in any direction but within 1 tile space
        if (newPos == pos + Vector2Int.right || 
        newPos == pos + Vector2Int.down || 
        newPos == pos + Vector2Int.up || 
        newPos == pos + Vector2Int.one || 
        newPos == pos - Vector2Int.one || 
        newPos == pos + Vector2Int.left || 
        newPos == pos + Vector2Int.right + Vector2Int.down || 
        newPos == pos + Vector2Int.left + Vector2Int.up) 
        {
            return true;
        } else {
