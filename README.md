...
* Inheritance - There were two types of inheritance used for this game. One was used for both the King and Knight pieces and the other for the pawn piece. This is because the pawn piece requires more information to be passed on such as the board and boolean movement value. The king and knight pieces although can use the same parameters for their movement, have different methods of movement: ie. knights go in a L and kings go in any compass direction. An interface was primarily chosen because both the King and Knight pieces had a common goal of checking to see if they could move to a particular tile using the given parameters. 
